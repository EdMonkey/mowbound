...
- Rocks & trees: destructible props with all-or-nothing damage — a swing only breaks one when its damage is strictly greater than the obstacle HP (test HP 5; base damage 3 won't break them, so damage upgrades are needed). A swing that fails to break it stuns the player for 1.5s (`obstacleStunSeconds`, a recoil); a successful break does not stun. Intact obstacles block movement via a collision circle (`OBSTACLE_BASE_RADIUS` per kind × the instance's visual scale, stored on the state as `radius`, resolved by `resolveCollision`); once broken they're passable. The obstacle swing is radius-aware (reach = attack range + obstacle radius) so a large rock is still hittable when the blade touches its edge. A broken rock leaves rubble (rock_broken model), a cut tree leaves a stump (tree_stump model), with grey/brown chip particles (`Debris`). Logic in `ObstacleSystem` (pure, reuses the attack fan); `Obstacle` is the visual. Scattered randomly per map (OBSTACLE_COUNTS_BY_MAP) on both maps; not yet wired to a skill-tree node.
