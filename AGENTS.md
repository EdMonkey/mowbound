...
- Rocks & trees: destructible props with all-or-nothing damage — a swing only breaks one when its damage is strictly greater than the obstacle HP (test HP 5; base damage 3 won't break them, so damage upgrades are needed). A swing that fails to break it stuns the player for 1.5s (`obstacleStunSeconds`, a recoil); a successful break does not stun. Intact obstacles block movement via a collision circle (`OBSTACLE_BASE_RADIUS` per kind × the instance's visual scale, stored on the state as `radius`, resolved by `resolveCollision`). The obstacle swing is radius-aware (reach = attack range + obstacle radius) so a large rock is still hittable when the blade touches its edge. On break: a **rock vanishes entirely and becomes passable** (grey chip particles); a **tree leaves a stump that keeps blocking** (collision shrinks to `TREE_STUMP_BASE_RADIUS`), throws a bigger/denser burst of wood chips, and spawns a `FallingLog` (the `tree_top` model) that topples sideways and fades out. Logic in `ObstacleSystem` (pure, reuses the attack fan); `Obstacle`/`FallingLog`/`Debris` are the visuals. Scattered randomly per map (OBSTACLE_COUNTS_BY_MAP) on both maps; not yet wired to a skill-tree node.
