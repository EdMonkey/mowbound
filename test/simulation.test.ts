...
describe("grass and coins", () => {
  const grassPatch = (id: string, x: number, z: number) => ({
    id,
    position: { x, z },
    hp: 5,
  ...
  it("temporarily disables grass chunk culling for result snapshots", () => {
    ...
    field.add(grassPatch("a", 0, 0));
    field.add(grassPatch("b", 8, 8));
  ...
  it("renders existing grass chunks without frustum culling while result snapshots render", () => {
    const field = new GrassField();
    field.add(grassPatch("alive-a", 0, 0));
    field.add(grassPatch("cut", 3, 3));
    field.add(grassPatch("alive-b", 8, 8));
    field.destroy("cut");
